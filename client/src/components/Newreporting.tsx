...
`;
  ...
  //   useEffect(() => {
  //     console.log('useEffect',secondCat);
  //     async function test(){
  //       console.log(firstCat)
  //       console.log(secondCat)
  //       console.log(thirdCat)
  //     let sum = await [...secondCat].map((e,i)=> ((firstCat[i] + e.count)/thirdCat[i].count).toFixed(2));
  //     console.log('sum effect', sum)
  //     setFourthCat(sum)
  //     }
  //     test();
  //     // 
  ...
  function calcData(data) {
    ...
    var sum = [...secondCat].map((e, i) => ( e.count / (firstCat[i] + thirdCat[i].count)).toFixed(2));
    setFourthCat(sum)
    ...
    <div className="reportContainer">
      ...
      {/* <Form className="reportingForm" onSubmit={handleSubmit(onSubmit)}>
        <Form.Group controlId="new">
          ...
          <Form.Control type="date" name="endDate" dateformat="MM-DD-YY" onChange={(e) => { setStartDate(e.target.value) }} defaultValue={moment(new Date(date) - ((24 * 60 * 60 * 1000) * 5)).format("YYYY-MM-DD")} ref={register({ required: true })} />
          {errors.date && errors.date.type === "required" && (
            <span role="alert">This is required</span>
          )}
          <Form.Label aria-invalid={errors.date ? "true" : "false"}>End Date</Form.Label>
          <Form.Control type="date" name="startDate" dateformat="DD-MM-YY" onChange={(e) => { setEndDate(e.target.value) }} defaultValue={date} ref={register({ required: true })} />
          {errors.date && errors.date.type === "required" && (
            <span role="alert">This is required</span>
          )}
        </Form.Group>
